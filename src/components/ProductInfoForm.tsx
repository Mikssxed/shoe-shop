...
import { Box, Button, Typography, Grid, InputLabel, Paper, IconButton } from '@mui/material'
import React, { useState } from 'react'
import { Dropdown, Input, TextArea } from './ui'
import theme from '@/theme'
import { Gallery } from 'iconsax-react'
import Image from 'next/image'
import { formAndImagesContainer, formSaveButton, imageCoverOnHover, imageUploadBox, imageUploadText, inputContainer, productImageContainer, productInfoFormContainer, productInfoTitle, productSizeButton, saveButtonContainer, trashIconContainer } from '@/styles/products/productInfoFormStyles'
...
const FormTitleAndDesc = ({ title, desc }: { title: string, desc: string }) => {
  return (
    <Box component="div">
      <Typography variant="h1" sx={{ pl: "0.28px" }}>{title}</Typography>
      <Typography
        variant="body1"
        sx={productInfoTitle}
      >
  ...
  return (
    ...
    >
      ...
      >
            ...
            <Grid key={item} item xs={6} lg={12} xl={6}>
              <Box sx={productImageContainer}>
                <Box sx={imageCoverOnHover}>
                  <IconButton sx={trashIconContainer} onClick={() => setIsDelete(true)}>
                    <Image
                      width={20}
                      height={20}
                      src={'/icons/trash.svg'}
        ...
        <Grid item xs={12} sm={productImages.length > 0 ? 6 : 12} lg={12} xl={6} padding={0}>
          <Paper elevation={0} sx={imageUploadBox}>
            <Gallery size="38" color={theme.palette.grey[500]} style={{ flexShrink: 0 }} />
            <Typography 
              variant="body1"
              sx={imageUploadText}>
